...
<section class="hero" id="heroSection">
            ...
            <?php foreach ($allSlides as $idx => $slide): ?>
                ...
                <?php else: ?>
                    ...
                    <div class="hero-bg-slide <?= $idx === 0 ? 'active' : ''; ?>" data-index="<?= $idx; ?>"
                        style="background-image: url('<?= base_url($slide['image_path']); ?>');"></div>
                <?php endif; ?>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
    ...
    <div class="container hero-content">
        ...
        <h2>The Essence of <br><span style="color: var(--color-gold); font-style: italic;">Purity & Tradition</span>
        </h2>
        <p>Experience the finest saffron, unadulterated honey, and hand-selected spices from the valleys of Kashmir.
            Curated by Gilaf Foods.</p>
        <div class="hero-buttons">
            <a href="<?= base_url('shop.php'); ?>" class="btn btn-primary"
                style="background: transparent; color: #FFFFFF; border: 2px solid #C5A059; border-radius: 8px; padding: 14px 32px; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 600; font-size: 0.85rem;">Shop
                Collection</a>
            <a href="#verification" class="btn btn-outline"
                style="background: transparent; color: #FFFFFF; border: 2px solid #C5A059; border-radius: 8px; padding: 14px 32px; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 600; font-size: 0.85rem;">Verify
                My Product</a>
        </div>
    </div>
                ...
                <?php foreach ($freshlyHarvestedProducts as $product): ?>
                    <article class="product-card" style="cursor: pointer;"
                        ...
                        <div class="product-image-wrapper"
                            onclick="event.stopPropagation(); trackClick(<?= $product['id']; ?>, 'homepage'); window.location.href='<?= base_url('product.php?id=' . $product['id']); ?>'"
            ...
            <?php if ($hasAdMedia): ?>
                <?php if ($adVideo): ?>
                    ...
                    <div id="adVideoWrapper" style="width: 100%; height: 100%; position: relative;">
                        <video id="adVideo" muted playsinline preload="auto" <?= empty($adImages) ? 'loop' : ''; ?>
