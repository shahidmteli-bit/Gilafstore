...
// Fetch Freshly Harvested products
...
// Fetch Explore Our Products (all products for discovery section)
...
<?php
// Fetch hero banner images from database (images only, content is fixed)
$heroImages = [];
$heroSliderEnabled = false;
$heroSliderTimer = 5;
$defaultSlideEnabled = true; // Default slide ON by default
...
try {
    ...
    foreach ($heroSettings as $hs) {
        if ($hs['setting_key'] === 'slider_enabled') $heroSliderEnabled = ($hs['setting_value'] === '1');
        if ($hs['setting_key'] === 'slider_timer') $heroSliderTimer = intval($hs['setting_value']);
        if ($hs['setting_key'] === 'default_slide_enabled') $defaultSlideEnabled = ($hs['setting_value'] === '1');
    }
} catch (PDOException $e) {
    // Tables may not exist - use default
}

// Build slides array - include default slide if enabled
$allSlides = [];
$slideIndex = 0;
...
<section class="hero" id="heroSection">
    ...
    <div class="hero-bg-slider" id="heroBgSlider" data-timer="<?= $heroSliderTimer; ?>" data-enabled="<?= $hasMultipleSlides ? '1' : '0'; ?>">
        <?php foreach ($allSlides as $idx => $slide): ?>
            <?php if ($slide['type'] === 'default'): ?>
            <!-- Default Slide (CSS background from style.css) -->
            <div class="hero-bg-slide hero-default-slide <?= $idx === 0 ? 'active' : ''; ?>" data-index="<?= $idx; ?>"></div>
            <?php else: ?>
            <!-- Uploaded Image Slide -->
            <div class="hero-bg-slide <?= $idx === 0 ? 'active' : ''; ?>" data-index="<?= $idx; ?>" style="background-image: url('<?= base_url($slide['image_path']); ?>');"></div>
            <?php endif; ?>
        <?php endforeach; ?>
    </div>
    <?php endif; ?>
    
    <!-- Fixed Content Layer (Never Changes) -->
    <div class="container hero-content">
        ...
        <div class="hero-buttons">
            <a href="<?= base_url('shop.php'); ?>" class="btn btn-primary" style="background: transparent; color: #FFFFFF; border: 2px solid #C5A059; border-radius: 8px; padding: 14px 32px; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 600; font-size: 0.85rem;">Shop Collection</a>
            <a href="#verification" class="btn btn-outline" style="background: transparent; color: #FFFFFF; border: 2px solid #C5A059; border-radius: 8px; padding: 14px 32px; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 600; font-size: 0.85rem;">Verify My Product</a>
        </div>
    </div>
    
    <?php if ($hasMultipleSlides): ?>
    <!-- Slider Navigation Dots -->
    <div class="hero-slider-dots" style="position: absolute; bottom: 30px; left: 50%; transform: translateX(-50%); display: flex; gap: 12px; z-index: 10;">
        <?php foreach ($allSlides as $idx => $slide): ?>
        <span class="hero-dot <?= $idx === 0 ? 'active' : ''; ?>" data-index="<?= $idx; ?>" style="width: 12px; height: 12px; border-radius: 50%; background: <?= $idx === 0 ? '#C5A059' : 'rgba(255,255,255,0.5)'; ?>; cursor: pointer; transition: all 0.3s;" title="<?= $slide['type'] === 'default' ? 'Default Slide' : 'Slide ' . ($idx + 1); ?>"></span>
        <?php endforeach; ?>
    </div>
    <?php endif; ?>
</section>
...
.hero.has-slider { background: #1a3c34 !important; }
...
.hero-bg-slider { 
    ...
    height: 100%; 
    z-index: 0;
    background: #1a3c34; /* Solid background to prevent any flash */
}
.hero-bg-slide {
    position: absolute;
    top: 0;
...
/* Default slide uses the same background as defined in style.css for .hero */
.hero-bg-slide.hero-default-slide {
    background: linear-gradient(rgba(26, 60, 52, 0.3), rgba(26, 60, 52, 0.6)), url('https://images.unsplash.com/photo-1596040033229-a9821ebd058d?q=80&w=2070&auto=format&fit=crop');
    background-size: cover;
    background-position: center;
}
...
.hero .hero-slider-dots { z-index: 10; }
</style>
...
<section class="products-section freshly-harvested-section" id="freshly-harvested">
    <div class="container">
        <div class="section-header-card">
            ...
            <p class="section-subtitle">Straight from the valleys of Kashmir</p>
        </div>
                ...
                <article class="product-card" style="cursor: pointer;" onclick="window.location.href='<?= base_url('product.php?id=' . $product['id']); ?>'">
                    ...
                    <div class="product-details">
                        ...
                        <div class="price-row">
                            ...
                            <?php } else {
                                $priceToConvert = !empty($product['has_discount']) ? $product['discounted_price'] : $priceRange['min_price'];
                    ...
                    <div class="product-details">
                        ...
                        <div class="price-row">
                            <?php 
                                $priceRange = get_product_price_range($product['id']);
                                if ($priceRange['has_range']) {
                                    // Show price range for products with multiple weights
                                    $minPrice = display_price($priceRange['min_price'], $currentCurrency, $currentCurrencySymbol);
                                    $maxPrice = display_price($priceRange['max_price'], $currentCurrency, $currentCurrencySymbol);
                            ?>
                                <span class="product-price"><?= $minPrice; ?> – <?= $maxPrice; ?></span>
                            <?php } else {
                                // Single price (no weight variants)
                                $priceToConvert = !empty($product['has_discount']) ? $product['discounted_price'] : $priceRange['min_price'];
                                $convertedPrice = display_price($priceToConvert, $currentCurrency, $currentCurrencySymbol);
                            ?>
                                <?php if (!empty($product['has_discount'])): ?>
...
<section class="story-section" id="story" style="display: flex; flex-wrap: wrap;">
    ...
    <div class="story-ad-screen" style="flex: 1 1 50%; min-height: 450px; display: flex; align-items: center; justify-content: center; padding: 40px; background: rgba(0,0,0,0.1);">
        <div class="ad-container" style="width: 100%; max-width: 450px; aspect-ratio: 4/3; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 40px rgba(0,0,0,0.3);">
            <?php if ($hasAdMedia): ?>
                <?php if ($adVideo): ?>
                <!-- Video Display -->
                <div id="adVideoWrapper" style="width: 100%; height: 100%; position: relative;">
                    <video id="adVideo" muted playsinline preload="auto" <?= empty($adImages) ? 'loop' : ''; ?> style="width: 100%; height: 100%; object-fit: cover;">
                        <source src="<?= base_url($adVideo['file_path']); ?>" type="video/mp4">
                    </video>
                    <!-- Mute/Unmute Button - Hidden by default, shows on hover -->
                    <button id="videoMuteBtn" type="button" style="position: absolute; bottom: 20px; right: 20px; width: 44px; height: 44px; border-radius: 50%; background: rgba(0,0,0,0.7); border: 2px solid rgba(255,255,255,0.3); cursor: pointer; display: flex; align-items: center; justify-content: center; z-index: 100; transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(0,0,0,0.4); opacity: 0; pointer-events: none;" title="Click to unmute">
                        <i id="muteIcon" class="fas fa-volume-mute" style="color: #fff; font-size: 18px;"></i>
                        <i id="unmuteIcon" class="fas fa-volume-up" style="color: #fff; font-size: 18px; display: none;"></i>
                    </button>
                    <style>
                        #adVideoWrapper:hover #videoMuteBtn { opacity: 1 !important; pointer-events: auto !important; }
                    </style>
                </div>
                <?php endif; ?>
                ...
                <!-- Images Display -->
                <div id="adImagesWrapper" style="width: 100%; height: 100%; position: relative; <?= $adVideo ? 'display:none;' : ''; ?>">
                    ...
                    <div class="ad-slider" id="adSlider" style="width: 100%; height: 100%; position: relative;">
                        ...
                        <div class="ad-slide" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; opacity: <?= $index === 0 ? '1' : '0'; ?>; transition: opacity 0.6s ease;">
                            <img src="<?= base_url($img['file_path']); ?>" alt="Advertisement" style="width: 100%; height: 100%; object-fit: cover;" loading="lazy">
                        </div>
                        <?php endforeach; ?>
                    </div>
                    <!-- Slider Dots -->
                    <div class="ad-slider-dots" style="position: absolute; bottom: 15px; left: 50%; transform: translateX(-50%); display: flex; gap: 8px; z-index: 10;">
                        <?php foreach ($adImages as $index => $img): ?>
                        <span class="ad-dot" data-index="<?= $index; ?>" style="width: 10px; height: 10px; border-radius: 50%; background: <?= $index === 0 ? '#C5A059' : 'rgba(255,255,255,0.5)'; ?>; cursor: pointer; transition: all 0.3s;"></span>
                        <?php endforeach; ?>
                    </div>
                    <?php else: ?>
                    <!-- Single Image Mode -->
                    <img src="<?= base_url($adImages[0]['file_path']); ?>" alt="Advertisement" style="width: 100%; height: 100%; object-fit: cover;" loading="lazy">
                    <?php endif; ?>
                </div>
                <?php endif; ?>
            <?php else: ?>
            <!-- Placeholder when no media uploaded -->
            <div class="ad-placeholder" style="width: 100%; height: 100%; background: linear-gradient(135deg, #1a3c34 0%, #0d1f1b 100%); display: flex; align-items: center; justify-content: center; color: rgba(255,255,255,0.3); font-size: 1rem; text-transform: uppercase; letter-spacing: 2px;">
