<?php
/**
 * WhatsApp OTP Login — Modal + JS only (no button).
 * The button is rendered inline in the login page.
 * Include this after the button to get the modal HTML + script.
 */
?>
...
<div id="waOtpModal" class="wa-otp-modal" style="display:none;">
    <div class="wa-otp-modal-content">
        ...
        <div class="wa-otp-header">
            <div class="wa-otp-icon"><i class="fab fa-whatsapp"></i></div>
        ...
        <div id="waOtpStep1" class="wa-otp-step">
            <div class="wa-otp-input-group">
                ...
                <input type="tel" id="waOtpPhone" class="wa-otp-phone" placeholder="Enter your mobile number" maxlength="10" pattern="[0-9]{10}" autocomplete="tel">
            </div>
        ...
        <div id="waOtpStep2" class="wa-otp-step" style="display:none;">
            ...
            <button type="button" class="wa-otp-submit" onclick="waOtpVerify()"><span>Verify & Login</span></button>
            <div class="wa-otp-resend">
                <span id="waOtpTimer">Resend in <span id="waOtpCountdown">60</span>s</span>
                <button type="button" id="waOtpResendBtn" style="display:none;" onclick="waOtpResend()">Resend Code</button>
            </div>
        ...
        <div id="waOtpLoading" class="wa-otp-loading" style="display:none;">
            <div class="wa-otp-spinner"></div><p>Please wait...</p>
        </div>
...
@media (max-width: 480px) { .wa-otp-modal-content { padding: 24px; margin: 10px; } .wa-otp-digit { width: 42px; height: 50px; font-size: 20px; } }
</style>
...
(function() {
    ...
    window.waOtpSendCode = async function() {
        const phone = document.getElementById('waOtpPhone').value.replace(/\D/g, '');
        if (phone.length !== 10) { waOtpShowError('Please enter a valid 10-digit mobile number'); return; }
        waOtpPhone = '+91' + phone;
        waOtpShowLoading(true);
        try {
            const res = await fetch('<?= base_url('api/whatsapp_otp.php') ?>', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ action: 'send', phone: waOtpPhone }) });
            ...
            } else { waOtpShowError(data.error || 'Failed to send OTP. Please try again.'); }
        } catch (err) { waOtpShowLoading(false); waOtpShowError('Network error.'); }
    };

    window.waOtpVerify = async function() {
        let otp = '';
        document.querySelectorAll('.wa-otp-digit').forEach(d => otp += d.value);
        if (otp.length !== 6) { waOtpShowError('Please enter the complete 6-digit code'); return; }
        waOtpShowLoading(true);
        try {
            const res = await fetch('<?= base_url('api/whatsapp_otp.php') ?>', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ action: 'verify', phone: waOtpPhone, otp: otp }) });
            const data = await res.json();
            if (data.success) { window.location.href = data.redirect || '<?= base_url('user/dashboard.php') ?>'; }
            else { waOtpShowLoading(false); waOtpShowError(data.error || 'Invalid OTP. Please try again.'); document.querySelectorAll('.wa-otp-digit').forEach(d => d.value = ''); document.querySelector('.wa-otp-digit').focus(); }
        } catch (err) { waOtpShowLoading(false); waOtpShowError('Network error.'); }
    };
    ...
    window.waOtpResend = async function() {
        waOtpShowLoading(true);
        try {
            const res = await fetch('<?= base_url('api/whatsapp_otp.php') ?>', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ action: 'resend', phone: waOtpPhone }) });
            const data = await res.json();
            waOtpShowLoading(false);
            if (data.success) { waOtpStartCountdown(60); waOtpHideError(); }
            else { if (data.retry_after) waOtpStartCountdown(data.retry_after); waOtpShowError(data.error || 'Failed to resend.'); }
        } catch (err) { waOtpShowLoading(false); waOtpShowError('Network error.'); }
    };
    ...
    function waOtpStartCountdown(seconds) {
        ...
        if (waOtpCountdownInterval) clearInterval(waOtpCountdownInterval);
        let remaining = seconds; countdownEl.textContent = remaining;
        waOtpCountdownInterval = setInterval(() => {
            remaining--; countdownEl.textContent = remaining;
            if (remaining <= 0) { clearInterval(waOtpCountdownInterval); timerEl.style.display = 'none'; resendBtn.style.display = 'inline'; }
        }, 1000);
    }

    function waOtpShowLoading(show) {
        document.getElementById('waOtpLoading').style.display = show ? 'block' : 'none';
        const s1 = document.getElementById('waOtpStep1'), s2 = document.getElementById('waOtpStep2');
        if (show) { s1.style.display = 'none'; s2.style.display = 'none'; }
        else { if (s2.dataset.active) { s2.style.display = 'block'; } else { s1.style.display = 'block'; } }
    }

    function waOtpShowError(msg) { const e = document.getElementById('waOtpError'); e.textContent = msg; e.style.display = 'block'; }
    function waOtpHideError() { document.getElementById('waOtpError').style.display = 'none'; }
    ...
    document.querySelectorAll('.wa-otp-digit').forEach((input, idx, arr) => {
        input.addEventListener('input', e => {
            const val = e.target.value.replace(/\D/g, ''); e.target.value = val.slice(0, 1);
            if (val && idx < arr.length - 1) arr[idx + 1].focus();
            if (idx === arr.length - 1 && val) { let all = true; arr.forEach(d => { if (!d.value) all = false; }); if (all) waOtpVerify(); }
        });
        input.addEventListener('keydown', e => { if (e.key === 'Backspace' && !e.target.value && idx > 0) arr[idx - 1].focus(); });
        input.addEventListener('paste', e => {
            e.preventDefault();
            const paste = (e.clipboardData || window.clipboardData).getData('text').replace(/\D/g, '');
            paste.split('').slice(0, 6).forEach((c, i) => { if (arr[i]) arr[i].value = c; });
            arr[Math.min(paste.length, 5)].focus();
            if (paste.length >= 6) waOtpVerify();
        });
    });

    document.addEventListener('keydown', e => { if (e.key === 'Escape' && document.getElementById('waOtpModal').style.display === 'flex') waOtpCloseModal(); });
    document.getElementById('waOtpModal')?.addEventListener('click', e => { if (e.target.id === 'waOtpModal') waOtpCloseModal(); });
})();
</script>
