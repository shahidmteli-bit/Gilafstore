        ...
        <div class="container">
            <div class="footer-grid">
                <div class="footer-brand">
                    <div class="logo" style="text-align: left; max-width: fit-content;">
                        <p style="font-family: var(--font-serif); font-size: 1.8rem; font-weight: 700; letter-spacing: 2px; margin-bottom: 5px; color: var(--color-white); line-height: 1;">GILAF STORE</p>
                        <span style="font-size: 0.6rem; letter-spacing: 3.5px; display: block; text-align: center; color: #D4B06A;">TASTE • CULTURE • CRAFT</span>
                    </div>
                    ...
                    <div class="social-icons" style="margin-top: 20px;">
                        <a href="https://www.instagram.com/gilafstore?igsh=MXN5eHg4emhmMWtmZQ==" target="_blank" aria-label="Instagram"><i class="fab fa-instagram"></i></a>
                        <a href="https://www.facebook.com/share/1D8xHnuELW/" target="_blank" aria-label="Facebook"><i class="fab fa-facebook-f"></i></a>
                        <a href="https://wa.me/919419404670" target="_blank" aria-label="WhatsApp"><i class="fab fa-whatsapp"></i></a>
                        <a href="https://www.linkedin.com/company/gilafstore/" target="_blank" aria-label="LinkedIn"><i class="fab fa-linkedin-in"></i></a>
                    </div>
                </div>
                ...
                <div class="footer-col">
                    ...
                    <ul>
                        ...
                        <li><a href="<?= base_url('user/create_ticket.php'); ?>">Contact Us</a></li>
                        <li><a href="<?= base_url('apply-distributor.php'); ?>">Become a Distributor</a></li>
            ...
            <div class="shipping-partners-section">
                <div class="ship-group">
                    <h4 class="shipping-title"><i class="fas fa-globe-americas"></i> International Shipping</h4>
                ...
                <div class="ship-group">
                    <h4 class="shipping-title"><i class="fas fa-truck"></i> Domestic Shipping (India)</h4>
    ...
    <div class="modal-overlay" id="loginModal">
        <div class="region-modal" style="width: 450px;">
            ...
            <div id="form-cust" class="form-section active" style="height: 380px; display: flex !important; flex-direction: column;">
                ...
                <form action="<?= base_url('distributor-login.php'); ?>" method="post" style="flex: 1; display: flex; flex-direction: column;">
                ...
                <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #e5e7eb; font-size: 0.85rem; color: #777;">
                    <div style="margin-bottom: 8px; text-align: center;">
                        <a href="<?= base_url('distributor-portal.php'); ?>" style="color: #1A3C34; text-decoration: none; font-weight: 600;"><i class="fas fa-key"></i> Forgot Password?</a>
                    </div>
    ...
    <!-- Scripts -->
    <script src="<?= asset_url_versioned('js/new-main.js'); ?>" defer></script>
        ...
        products.forEach(product => {
            ...
            html += `
                <a href="${product.url}" class="autocomplete-item" style="display: flex; align-items: center; padding: 12px 16px; text-decoration: none; color: inherit; border-bottom: 1px solid #f0f0f0; transition: background 0.2s ease;">
                    <img src="${imageUrl}" alt="${product.name}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 8px; margin-right: 12px;" onerror="this.style.background='#f3f4f6';this.style.objectFit='none';this.alt='📦'">
    ...
    <!-- Chatbot — lazy-loaded on scroll/interaction to save 89KB from initial load -->
    <script>
    (function(){var loaded=false;function loadChatbot(){if(loaded)return;loaded=true;
    var css=document.createElement('link');css.rel='stylesheet';css.href='<?= asset_url_versioned("css/chatbot.css"); ?>';document.head.appendChild(css);
    var js=document.createElement('script');js.src='<?= asset_url_versioned("js/chatbot.js"); ?>';document.body.appendChild(js);
    window.removeEventListener('scroll',onScroll);}
    function onScroll(){if(window.scrollY>window.innerHeight*0.5)loadChatbot();}
...
<!-- PWA: Cleanup stale service worker caches -->
<script>
...
<?php
// Record server-side page processing time
if (isset($_SERVER['REQUEST_TIME_FLOAT']) && function_exists('db_query')) {
    $serverTime = round(microtime(true) - $_SERVER['REQUEST_TIME_FLOAT'], 4);
    $pageUrl = substr($_SERVER['REQUEST_URI'] ?? '/', 0, 500);
    try {
        @db_query(
            "INSERT INTO page_performance (page_url, load_time, server_time, source, created_at) VALUES (?, ?, ?, 'server', NOW())",
            [$pageUrl, $serverTime, $serverTime]
        );
    } catch (\Exception $e) {
        ...
        try {
            @db_query("CREATE TABLE IF NOT EXISTS page_performance (
                ...
                source ENUM('server','client') DEFAULT 'server',
...
<!-- Page Performance Beacon (client-side full load time) -->
<script>
    ...
    setTimeout(function() {
        try {
            ...
            if (entries && entries.length > 0) {
                var nav = entries[0];
                var loadTime = (nav.loadEventEnd - nav.startTime) / 1000;
                if (loadTime > 0 && loadTime < 60 && navigator.sendBeacon) {
                    var payload = JSON.stringify({
                        url: window.location.pathname,
                        load_time: loadTime,
                        ttfb: (nav.responseStart - nav.startTime) / 1000,
                        dom_ready: (nav.domContentLoadedEventEnd - nav.startTime) / 1000
                    });
                    navigator.sendBeacon('<?= base_url("api/performance_beacon.php"); ?>', new Blob([payload], {type: 'application/json'}));
                }
            }
        } catch(e) {}
    }, 200);
...
<!-- SEO: Organization + WebSite structured data (global, all pages) -->
<?php if (function_exists('build_organization_schema')): ?>
<script type="application/ld+json"><?= json_encode(build_organization_schema(), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?></script>
<script type="application/ld+json"><?= json_encode(build_website_schema(), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE); ?></script>
<?php endif; ?>
</body>
</html>
