...
// ─── Admin Gate Check ─────────────────────────────────────────
// Block direct GET access to admin login (what scanners/bots do).
// Allow access if:
// 1) Already logged in as admin, OR
// 2) Has a valid gate token from /gs-secure-portal-92XK.php (valid 30 min), OR
// 3) Has a pending MFA verification (mid-login flow), OR
// 4) Request is POST (existing distributor login forms — protected by rate limiting)
$hasGateToken = !empty($_SESSION['_admin_gate_token']) 
    && !empty($_SESSION['_admin_gate_time']) 
    && (time() - $_SESSION['_admin_gate_time'] < 1800);
$hasPendingMFA = !empty($_SESSION['_2fa_pending_admin']);
$isPostRequest = ($_SERVER['REQUEST_METHOD'] === 'POST');

if (!is_admin() && !$hasGateToken && !$hasPendingMFA && !$isPostRequest) {
    // Log the unauthorized access attempt
    try {
        security_log('ADMIN_DIRECT_ACCESS_BLOCKED', 'WARNING', 
            'Direct admin login access blocked — no gate token', [
    ...
    // Return a genuine 404 — do not reveal admin page exists
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        ...
        if (attempt_login($identifier, $password, 'admin')) {

            $mfaMethod = admin_pending_mfa_method();

            if ($mfaMethod === 'totp') {

                session_write_close();

                header('Location: ' . base_url('admin/two_factor_verify.php'));

                exit;

            } elseif ($mfaMethod === 'passkey') {

                session_write_close();

                header('Location: ' . base_url('admin/passkey_verify.php'));

                exit;

            } elseif ($mfaMethod === 'email_otp') {

                // Send OTP email now

                $pendingAdmin = $_SESSION['_2fa_pending_admin'];

                $otpStatus = admin_email_otp_status($pendingAdmin['id']);

                $secEmail = $otpStatus['security_email'] ?? $pendingAdmin['email'];

                $otp = generate_email_otp();

                send_email_otp($secEmail, $otp);

                $_SESSION['_email_otp_target'] = $secEmail;

                session_write_close();

                header('Location: ' . base_url('admin/email_otp_verify.php'));
            ...
            // No need to unset($_SESSION['user']) here as we are targeting admin session
