...
try {
    ...
    // Fire WACRM order.cancelled event (non-blocking)
    try {
        require_once __DIR__ . '/../includes/ce_core/WACRMPublisher.php';
        (new CE_WACRMPublisher())->publishOrderStatus('cancelled', $orderId, [
            'cancel_reason' => $reason,
        ]);
    } catch (\Throwable $wacrmErr) {
        error_log("WACRM publisher failed for cancelled order #{$orderId}: " . $wacrmErr->getMessage());
    }
